    ...
    <div class="card card-success">
      ...
      <div class="card-body">
        ...
        <div class="row">
          ...
          <div class="col-md-4">
            <div class="form-group">
              <?php echo Form::label('travellers',__('Number of cars'), ['class' => 'form-label']); ?>

                <input class="form-control" type="number" id="car" name="car" value="1">
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-md-4">
            <div class="form-group">
              <?php echo Form::label('vehicle_id',__('Select Fare'), ['class' => 'form-label']); ?>

              <select id="fare_id" name="fare_id" class="form-control" required>
                <option value="">-</option>
                ...
                <option value="<?php echo e($row->intercity_fare_id); ?>" data-fare="<?php echo e($row->fare); ?>"><?php echo e($row->district_name); ?> , <?php echo e($row->upozila_name); ?> --- <?php
                  $to= DB::table('thana')
          ...
          <div class="col-md-2">
            <div class="form-group">
                <br><input style="margin-top: 17px;" type="checkbox" id="return" name="return_status" value="1">
              <label for="vehicle1"> Return</label> <span style="margin-left: 20px;color: red;" id="return_info"></span>
              <input class="form-control" type="hidden" id="return_amount" name="return_amount" value="0.00" readonly>
            </div>
          </div>
          <div class="col-md-2">
            <div class="form-group">
              <?php echo Form::label('travellers',__('Base Fare'), ['class' => 'form-label']); ?>

              <input class="form-control" type="number" id="base_trip_fare" name="base_trip_fare" value="0.00" readonly>
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-group">
              <?php echo Form::label('travellers',__('Total Fare'), ['class' => 'form-label']); ?>

              <input class="form-control" type="number" id="trip_fare" name="trip_fare" value="0.00" readonly>
            </div>
          </div>
        </div>
        <?php if(Auth::user()->user_type == "C"): ?>
...
<script type="text/javascript">
  ...
  $('#fare_id').select2({placeholder: "Select Fare"});
  ...
  $(document).ready(function() {
    ...
    $("#fare_id").on("change",function(){
      var fare = $(this).find(":selected").data("fare");
      var car = $("#car").val();
      var total_fare=car*fare;
      $("#trip_fare").val(total_fare).change();
      $("#base_trip_fare").val(fare).change();
    });
    $("input[name=car]").change(function(){
      var fare_id = $("#fare_id").val();
      var car = $("#car").val();
      var return_fare = $("#return").val();
      $.ajax({
        type: "POST",
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        url: "<?php echo e(url('admin/fare')); ?>",
        data: "fare_id="+fare_id,
        success: function(data2){
          console.log(data2.return_fare);
          if(return_fare==1){
            var return_extra=car*data2.return_fare;
            $("#trip_fare").val(data2.fare*car).change();
            $("#base_trip_fare").val(data2.fare).change();
            $('#return_info').html();
          }else{
            var return_extra=car*data2.return_fare;
            $("#trip_fare").val((data2.fare*car)+return_extra).change();
            $("#base_trip_fare").val(data2.fare).change();
            $('#return_info').html(return_extra+" TK extra fare !!");
          }
          ...
          $.ajax({
            ...
            success: function(data2){
              //add
                ...
                $("#trip_fare").val((data2.fare*car)+return_extra).change();
                $("#base_trip_fare").val(data2.fare).change();
                $("#return_amount").val(return_extra).change();
                $("#return").val(2).change();
                $('#return_info').html(return_extra+" TK extra fare !!");
            },
            dataType: "json"
  ...
  //       //add
  //         var return_extra=car*data2.return_fare;
  //         $("#trip_fare").val(((data2.fare*car)*return_fare)+return_extra).change();
  //         $("#base_trip_fare").val(data2.fare).change();
  //     },
  //     dataType: "json"
  //   });
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\BirthCertificate\framework\resources\views/bookings/create.blade.php ENDPATH**/ ?>