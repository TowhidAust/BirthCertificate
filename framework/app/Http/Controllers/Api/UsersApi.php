...
class UsersApi extends Controller {
	...
	public function picupThana($district_id) {
		$data= DB::table('thana')
								 ->where('city_id',$district_id)
								 ->select('id','upozila_name')
								 ->orderBy('upozila_name','asc')
								 ->get();
		return response()->json($data);
	}
	...
	public function dropThana($district_id) {
		$data= DB::table('thana')
								 ->where('city_id',$district_id)
								 ->select('id','upozila_name')
								 ->orderBy('upozila_name','asc')
	...
	public function guestCar() {
	$data= DB::table('car_slot as c')
							 ->join('vehicle_types', 'c.v_type_id', '=', 'vehicle_types.id')
							 ->join('time_slot', 'c.t_slot_id', '=', 'time_slot.id')
							 ->select('c.v_type_id as car_id',
							  'c.rent',
							  'vehicle_types.displayname as car_name',
							  'vehicle_types.icon as car_image',
							  'c.id as slot_id',
							  'time_slot.time as slot_time')
							 ->orderBy('vehicle_types.id', 'ASC')
							 ->get();
		return response()->json($data);
	}
	public function colors() {
		...
		public function carSlot() {
		$slot_rent= DB::table('car_slot')
                                   ->join('time_slot', 'car_slot.t_slot_id', '=', 'time_slot.id')
                                   ->select('*','car_slot.id as slot_id')
                                   ->get();
		return response()->json($slot_rent);
	}
	public function searchCarList(Request $request) {
				$trip_date =  $request->get('trip_date');
				$time =  $request->get('trip_time');
				$pickup_location =  $request->get('pickup_location');
				$from_thana =  DB::table('thana')
										->select('*')
		                ->where('id',$request->get('from_thana'))
										->first();
				$to_thana =  DB::table('thana')
										->select('*')
		                ->where('id',$request->get('to_thana'))
										->first();
				$fare;
				$fare= DB::table('intercity_fare as inf')
		                ->join('thana', 'inf.from_thana_id', '=', 'thana.id')
		                ->join('vehicle_types', 'inf.vehicle_type_id', '=', 'vehicle_types.id')
		                ->select('inf.id','inf.from_thana_id','inf.to_thana_id','inf.fare','inf.return_extra_fare','inf.vehicle_type_id','vehicle_types.vehicletype','vehicle_types.icon')
		                ->where('inf.from_thana_id',$request->get('from_thana'))
		                ->where('inf.to_thana_id',$request->get('to_thana'))
		                ->orderBy('inf.id','desc')
		                ->get();
		          if(sizeof($fare)<=0){
                 $fare= DB::table('intercity_fare as inf')
                ...
                ->join('vehicle_types', 'inf.vehicle_type_id', '=', 'vehicle_types.id')
                ->select('inf.id','inf.from_thana_id','inf.to_thana_id','inf.fare','inf.return_extra_fare','inf.vehicle_type_id','vehicle_types.vehicletype','vehicle_types.icon')
                ->where('inf.to_thana_id',$request->get('from_thana'))
                ->where('inf.from_thana_id',$request->get('to_thana'))
                ->orderBy('inf.id','desc')
                ->get();
		         }
		         $data = array(
				'from'=>$from_thana->upozila_name,
				'to'=>$to_thana->upozila_name,
				'trip_date'=>$trip_date,
				'time'=>$time,
	...
	public function book(Request $request) {
		 // return $request->all();
		$return_fare=$request->get("car")*$request->get("return_fare");
		$trip_fare=$request->get("car")*$request->get("base_fare");
		$total_fare=$return_fare+$trip_fare;
		...
		DB::table('bookings')->insert(
						...
						'trip_date' =>$request->get("trip_date"),
						'pickup' =>$request->get("trip_time"),
						'created_at' =>date("Y-m-d h:i:s"),
						'pickup_addr' =>$request->get("pickup_location")]
				);
			 $booking_id = DB::getPDO()->lastInsertId();
		 DB::table('bookings_meta')->insert(
					 ['type' => 'string',
        ...
        $this->sendPushNotification($customer_id);
		return response()->json($arrayName = array('message' =>'Your Booking complete Successfully!'  ));
	}
	...
	public function bookWedding(Request $request) {
		$wedding_pack_id=$request->get('wedding_pack_id');
	 ...
	 DB::table('bookings')->insert(
					 ...
					 'customer_id' =>$customer_id,
					 'trip_fare' =>$price,
					 'wedding_package_id' =>$wedding_pack_id,
					 'trip_date' =>$trip_date,
					 'type' =>'Wedding',
					 'decu_type' =>$type,
					 'pickup_addr' =>$pickup_location,
					 'user_id' =>'1',
					 'color_id' =>$color_id,
					 'created_at' =>date("Y-m-d h:i:s")]
			 );
	...
	public function bookGuestCar(Request $request) {
			$slot_ids = $request->slot;
		$car = $request->qty;
		// $s_id=0;
		// $qty=0;
		//  foreach($slot_ids as $key) {
		// 	 $s_id=$s_id+$key['slot_id'];
		// 	 $qty=$qty+$key['qty'];
		//
		//  }
		//
		// 	 return response()->json([
		// 			 'message' => $s_id,'qty'=>$qty
		// 	 ]);

			$wedding_pack_id=$request->wedding_pack_id;
	 		$price=$request->price;
	    ...
	    $customer_id=$request->customer_id;
	 	 ...
	 	 foreach($slot_ids as $key) {
			   $slot_id=$key['slot_id'];
	 			$slot_rent= DB::table('car_slot')
	 									->join('time_slot', 'car_slot.t_slot_id', '=', 'time_slot.id')
	 									->where('car_slot.id',$slot_id)
	 									->select('*','car_slot.id as slot_id')
	 									->first();
	 			$slot_total = $slot_total+($slot_rent->rent*$key['qty']);
	 			// $slot_time = $slot_rent->time;
	 			$total_car = $total_car+$key['qty'];
	 			//$car[$key];
	 			// $car_name[$key];
	 	 }
		 if($wedding_pack_id){
	 	 ...
	 	 $total_rent=$slot_total+$price;
	 	 DB::table('bookings')->insert(
	 					 ['car' => $total_car,
	 					 'customer_id' =>$customer_id,
	 					 'trip_fare' =>$total_rent,
	 					 'wedding_package_id' =>$wedding_pack_id,
	 					 'trip_date' =>$trip_date,
	 					 'type' =>'Wedding',
	 					 'decu_type' =>$type,
		    ...
		    DB::table('bookings_meta')->insert(
					 ...
					 'created_at' =>date("Y-m-d h:i:s")]
	 				 ...
	 				 foreach($slot_ids as $key) {
	 				DB::table('guest_car')->insert(
	 								['booking_id' => $booking_id,
	 								'car_slot_id' =>$key['slot_id'],
	 								'car' =>$key['qty'],
	 								'created_at' =>date("Y-m-d h:i:s")]
	 						);
	 			 }
	...
	public function bookGuestCarOnly(Request $request) {
	...
	public function userProfile(Request $request) {
		$user = User::find($request->user_id);
			$data['user_info']= array('user_id' => $user->id ,
			 'user_name' => $user->name,
			 'mobno' =>$user->getMeta('mobno'),
			 'emailid' =>$user->email,
			 'gender' =>$user->getMeta('gender'),
			 'profile_pic' =>$user->getMeta('profile_pic'),
			 'status' =>$user->getMeta('status'),
		    'phone_code'=>$user->getMeta('phone_code'));
			return $data;
	}
	...
	private function sendPushNotification($customer_id) {
	 $info = User::find($customer_id);
	 $header='Welcome To Highway360';
	 $message_body=' We got your booking request we will contact to you shortly';
	 $footer='Thank You '.$info->first_name.' '.$info->last_name;
	 $device_token='ceZ5W6jJRg-7Y0q_4dC9R0:APA91bHZle38ba0fPp_6luv2bCtw2AttevCriRdnfj2gbSSlt1sKeXEj3TuX_3phdSfFzTU-nBFmanrB9QbtB1XG3_B9YCCIXIostRu5LuFqC5hu4pKGDn5FlzTo93nnxdScBgKgrmJS';
		 $fields = array(
				'to' => $info->device_token,
				'notification' =>$notification = array('body' => 'Hello body','title'=>'highway360' ),
