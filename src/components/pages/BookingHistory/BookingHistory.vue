<template>
  <div>
    ...
    <section class="booking-scoped">
      <b-container>
        <h3 class="text-center font-weight-bold mb-4 text-capitalize">{{ $t('nav.bookingHistory') }}</h3>
        <b-form-group>
          ...
          <date-range-picker ref="picker" v-model="dateRange" opens="center">
            ...
            >
              <div v-if="picker.startDate" class="d-flex justify-content-center">
                <span>{{ picker.startDate | moment(dateFormat) }} &nbsp;To &nbsp; {{ picker.endDate | moment(dateFormat) }}</span>
                <span
              ...
              <div v-else style="text-align:center">{{$t('bookingHistory.searchText')}}</div>
            </div>
          </date-range-picker>
        </b-form-group>
          ...
          <b-col sm="12">
            ...
            <div v-if="searchResults.length">
              <h6 class="mb-5">
                ...
                {{$t('bookingHistory.searchTitle')}} {{dateRange.startDate | moment(dateFormat)}} {{$t('bookingHistory.to')}} {{dateRange.endDate | moment(dateFormat)}}
              </h6>
              <BookingHistoryItem
